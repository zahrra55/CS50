...
int main(void)
{
    ...
    // Convert play1 to uppercase and store in p1 array
    char p1[p1_len + 1];
    for (int j = 0; j < p1_len; j++)
    {
        p1[j] = toupper(play1[j]);
    }
    p1[p1_len] = '\0'; // Null-terminate the string

    // Convert play2 to uppercase and store in p2 array
    char p2[p2_len + 1];
    for (int j = 0; j < p2_len; j++)
    {
        p2[j] = toupper(play2[j]);
    }
    p2[p2_len] = '\0'; // Null-terminate the string

    // Calculate the sum for player 1
    ...
    // Calculate the sum for player 2
